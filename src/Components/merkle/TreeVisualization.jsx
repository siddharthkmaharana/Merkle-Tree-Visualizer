...
export default function TreeVisualization({ blocks, tamperMode, originalHashes, verifyIndex, isDark }) {
  ...
  const renderConnections = () => {
    ...
    for (let levelIndex = 0; levelIndex < levels.length - 1; levelIndex++) {
      ...
      const currentStart = (baseWidth - currentWidth) / 2;
      const nextStart = (baseWidth - nextWidth) / 2;

      nextLevel.forEach((parentNode, parentIndex) => {
        const leftChildIndex = parentIndex * 2;
        const rightChildIndex = parentIndex * 2 + 1;

        const parentX = nextStart + (parentIndex + 0.5) * (nextWidth / nextLevel.length);
        const parentY = (levels.length - levelIndex - 2) * levelHeight;

        // Left child connection
        if (leftChildIndex < currentLevel.length) {
          const childX = currentStart + (leftChildIndex + 0.5) * (currentWidth / currentLevel.length);
          const childY = (levels.length - levelIndex - 1) * levelHeight;
        ...
        if (rightChildIndex < currentLevel.length) {
          const childX = currentStart + (rightChildIndex + 0.5) * (currentWidth / currentLevel.length);
          const childY = (levels.length - levelIndex - 1) * levelHeight;
